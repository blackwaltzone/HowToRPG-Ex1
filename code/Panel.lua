...
function Panel:Create(params)
	...
	-- So we create a variable that will account for that when scrolling
...
function Panel:Position(left, top, right, bottom)
	...
	local widthScale = (math.abs(right - left) - self.mTileSize)
		 / self.mTileSize
	...
	local heightScale = (math.abs(bottom - top) - self.mTileSize)
		/ self.mTileSize
